...
const SectionHeading = ({
  ...
  const renderTitle = () => {
    switch (variant) {
      case 'modern':
        return (
          <h2 id={id} className="relative mb-3 text-3xl md:text-4xl font-bold">
            {title}
      ...
      case 'gradient':
        return (
          <h2 id={id} className="mb-3 text-3xl md:text-4xl font-bold bg-gradient-to-r from-mylli-primary to-mylli-secondary bg-clip-text text-transparent">
            {title}
          </h2>
        );
      case 'underline':
        return (
          <h2 id={id} className="relative inline-block mb-3 text-3xl md:text-4xl font-bold">
            {title}
      ...
      case 'split':
        ...
        return (
          <h2 id={id} className="section-title animate-fade-in">
            {parts[0]}
      ...
      case 'outlined':
        return (
          <h2 id={id} className="mb-3 text-3xl md:text-4xl font-bold">
      ...
      case 'animated':
        if (!highlightText) {
          return (
            <h2 id={id} className="mb-3 text-3xl md:text-4xl font-bold relative overflow-hidden">
        ...
        return (
          <h2 id={id} className="section-title mb-3 text-3xl md:text-4xl font-bold">
            {animatedParts[0]}
  ...
  return (
    <div className={`${alignmentClasses[align]} ${className}`}>
      {renderTitle()}
      {subtitle && (
        <p className="text-lg text-mylli-gray mt-2 max-w-3xl">
          {subtitle}
        </p>
      )}
      {children}
