      ...
      case 'native':
        if (navigator.share) {
          navigator.share({
            ...
            text: shareData.text,
            url: shareData.url
          }).then(() => toast.success('Contenu partagé avec succès')).catch(() => toast.error('Le partage a été annulé ou n\'a pas pu être effectué'));
        } else {
  ...
  return <div className="pt-16 md:pt-24">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-mylli-primary/5 to-mylli-accent/5 py-16">
        <div className="container-custom max-w-4xl">
          <Link to="/articles" className="inline-flex items-center text-mylli-primary mb-6 hover:text-mylli-primary-dark transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            ...
            <div className="rounded-xl overflow-hidden mb-8 shadow-md">
              <img src={article.imageSrc} alt={article.imageAlt} className="w-full h-auto object-cover aspect-video" />
            </div>
            
            {/* Article Content */}
            <div className="prose prose-lg max-w-none prose-headings:text-mylli-dark prose-p:text-mylli-gray prose-a:text-mylli-primary prose-a:no-underline hover:prose-a:text-mylli-primary-dark prose-a:transition-colors" dangerouslySetInnerHTML={{
            ...
            <div className="mt-12 pt-8 border-t">
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => handleShare('facebook')} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#3b5998] text-white hover:bg-[#3b5998]/90 transition-colors" aria-label="Partager sur Facebook">
                  <Facebook size={18} />
                  <span className="hidden sm:inline">Facebook</span>
                </button>
                <button onClick={() => handleShare('twitter')} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#1DA1F2] text-white hover:bg-[#1DA1F2]/90 transition-colors" aria-label="Partager sur Twitter">
                  <Twitter size={18} />
                  <span className="hidden sm:inline">Twitter</span>
                </button>
                <button onClick={() => handleShare('linkedin')} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#0077B5] text-white hover:bg-[#0077B5]/90 transition-colors" aria-label="Partager sur LinkedIn">
                  <Linkedin size={18} />
                  <span className="hidden sm:inline">LinkedIn</span>
                </button>
                <button onClick={() => handleShare('whatsapp')} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#25D366] text-white hover:bg-[#25D366]/90 transition-colors" aria-label="Partager sur WhatsApp">
                  <Instagram size={18} />
                  <span className="hidden sm:inline">WhatsApp</span>
                </button>
                <button onClick={() => handleShare('native')} className="flex items-center gap-2 px-4 py-2 rounded-full bg-mylli-primary text-white hover:bg-mylli-primary-dark transition-colors" aria-label="Plus d'options de partage">
                  <Share size={18} />
          ...
          <div className="lg:col-span-4">
            ...
            <div className="sticky top-24">
              ...
              <div className="space-y-6">
                {relatedArticles.map(relatedArticle => <Link to={`/articles/${relatedArticle.slug}`} key={relatedArticle.id} className="group">
                    <Card className="overflow-hidden hover:shadow-md transition-all duration-300 border border-transparent hover:border-mylli-primary/20 bg-white">
                      <div className="flex">
                        <div className="w-24 h-24 overflow-hidden">
                          <img src={relatedArticle.imageSrc} alt={relatedArticle.imageAlt} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                        </div>
              ...
              <Link to="/articles" className="mt-6 inline-flex items-center text-mylli-primary font-medium hover:text-mylli-primary-dark transition-colors">
        ...
        <div className="container-custom">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <div className="flex justify-center gap-4">
              <Link to="/contact" className="bg-white text-mylli-primary hover:bg-mylli-light py-3 px-6 rounded-full font-medium transition-colors">
                Nous contacter
              </Link>
              <Link to="/services" className="bg-white/20 hover:bg-white/30 text-white py-3 px-6 rounded-full font-medium transition-colors">
                Nos services
              </Link>
            </div>
          </div>
